...
<title>
<c:choose>
	<c:when test="${sessionScope.lang == 'ar'}"> : قائمة الطلبات</c:when>
	<c:when test="${sessionScope.lang == 'fr'}">Liste des Commandes :</c:when>
	<c:otherwise> list Commands :</c:otherwise>
</c:choose>
		...
		<nav
			...
			<div class="container-fluid">
				<div class="navbar-wrapper">
					...
					<a class="navbar-brand" href="#pablo"><c:choose>
	<c:when test="${sessionScope.lang == 'ar'}"> : قائمة الطلبات</c:when>
	<c:when test="${sessionScope.lang == 'fr'}">Liste des Commandes :</c:when>
	<c:otherwise> List Commands :</c:otherwise>
</c:choose> </a>
							...
							<div class="card-body">
								...
								<div class="material-datatables">
									...
									<table id="datatables"
										...
										<thead>
										<c:choose>
	<c:when test="${sessionScope.lang == 'ar'}"><tr>
												<th>طلب</th>
												<th>منطقة الضابط المسؤول</th>
												<th>اسم الزبون</th>
												<th>الكلفة</th>
												<th>الحالة</th>
												<th>امكانيات</th>
											</tr></c:when>
	<c:when test="${sessionScope.lang == 'fr'}"><tr>
												<th>la Commande</th>
											...
											<c:forEach items="${listCommande}" var="row">
												<tr>
													...
													<td><c:if
															...
															<a
																href="Servlet?action=BienRecu&id=${row.getCommande().getId()}"><i
																class="material-icons" style="color: green;">check</i></a>
														</c:if>
															
														</c:if>
														<c:if
															test="${sessionScope.type_account == 'admin'}">
															<a
																href="Servlet?action=ReadCommande&id=${row.getCommande().getId()}"><i
																class="material-icons" style="color: green;">chrome_reader_mode</i></a>
														</c:if>
