...
<title>
<c:if test="${sessionScope.lang == 'en'}">Package list </c:if>
	<c:if test="${sessionScope.lang == 'ar'}">قائمة الموزع</c:if> <c:if
		test="${sessionScope.lang == 'fr'}"> Liste des colis</c:if></title>
<link
		...
		<nav
			...
			<div class="container-fluid">
				<div class="navbar-wrapper">
					...
					<a class="navbar-brand" href="#pablo"><c:if
							test="${sessionScope.lang == 'en'}">List of commands </c:if> <c:if
							test="${sessionScope.lang == 'ar'}">قائمة الطلبات</c:if> <c:if
							test="${sessionScope.lang == 'fr'}">Liste des colis</c:if> </a>
				</div>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header card-header-primary card-header-icon">
								...
								<h4 class="card-title">
									<c:if test="${sessionScope.lang == 'en'}">Parcel </c:if>
									<c:if test="${sessionScope.lang == 'ar'}">الطلبات</c:if>
									<c:if test="${sessionScope.lang == 'fr'}">Colis</c:if>
								</h4>
							</div>
							<div class="card-body">
								<div class="toolbar">
								...
								<div class="material-datatables">
									...
									<table id="datatables"
										...
										<tbody>
											<c:forEach items="${listColis}" var="row">
												<tr>
													<c:if test="${sessionScope.lang == 'en'}">
														<td>${row.getNature()}</td>
														<td>${row.getPoids()}</td>
														<td>${row.getId_commande()}</td>
